...
from .commands import (
    c_status,c_mode,c_auto,c_plan,c_review,make_run,c_route,c_models,c_learning,
    c_checkpoint,c_resume,c_history,c_rescue,c_abort,c_help
)
...
def register(ctx):
    ...
    # Slash commands
    cmds=[
      ("cmd-status",c_status,"Show durable run status.","[--tasks|--json]"),
      ...
      ("cmd-plan",c_plan,"Create and save a plan without executing.","<work>"),
      ("cmd-review",c_review,"Review the current saved plan.",""),
      ("cmd-run",make_run(ctx),"Approve current plan and start execution.",""),
      ("cmd-route",c_route,"Show routing choice for a task.","<task>"),
      ("cmd-models",c_models,"Show logical model map.",""),
    ...
    # Hooks. All callbacks accept **kwargs for Hermes forward compatibility.
    def pre_llm_call(**kw):
        st=load_settings()
        r=STORE.current()
        if r:
            STORE.checkpoint(r["run_id"],"pre_llm_call",{"session_id":kw.get("session_id","")})
        auto=st.get("auto","review")
        if auto=="off":
            return None
        msg=(kw.get("user_message") or "").strip()
        if not msg or msg.startswith("/cmd-"):
            return None
        # Context injection only; the agent remains in control of tool execution.
        if auto=="review":
            return {"content":"cmd-orchestrator AUTO=review: for substantial work, call cmd_plan/cmd_orchestrate first and show the plan before execution. Do not execute until approved with /cmd-run."}
        if auto=="on":
            return {"content":"cmd-orchestrator AUTO=on: for substantial work, call cmd_orchestrate, follow its DAG and routing hints, checkpoint/verify tasks, and avoid rerunning DONE tasks."}
        return None
    ...
    # Best-effort extra hook for Hermes builds that expose provider request errors.
    def api_request_error(**kw):
        text=str(kw)
        r=STORE.current()
        if r:
            STORE.checkpoint(r["run_id"],"api_request_error",{"error":text[-2000:]})
            if looks_limited(text) and load_settings().get("fm_rescue",True):
